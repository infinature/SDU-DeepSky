            ...
            for row_number, data_row in enumerate(reader, 1):
                ...
                try:
                    ...
                    if not all([tile_id, ra_val, dec_val, filter_id_val]):
                        print(f"警告 (数据行 {row_number}): 缺少 TILE_ID, RA, DEC, 或 FILTER_ID 中的一个或多个值。跳过此行。")
                        failed_downloads +=1
                        continue
                    
                    print(f"处理行 {row_number}: 下载 PSF for TILE_ID={tile_id}, RA={ra_val}, Dec={dec_val}, Filter={filter_id_val}")
