...
# J-PLUS DR3 PSF 文件下载的基础URL
BASE_URL = "https://archive.cefca.es/catalogues/vo/siap/jplus-dr3/get_psf_file"

def download_jplus_psf(session, image_id, output_directory="."):
    """
    从 J-PLUS DR3 存档下载指定的PSF文件。
    ...
    try:
        ...
        # 假设PSF文件通常以 .psf 结尾，如果不是，需要根据实际情况调整
        filename = f"jplus_psf_id{image_id}.psf" 
        ...
        print(f"HTTP 错误下载 PSF ID {image_id}: {http_err}")
        if response is not None:
             print(f"  请求URL: {response.url}")
    except requests.exceptions.RequestException as req_err:
        print(f"请求错误下载 PSF ID {image_id}: {req_err}")
    except Exception as e:
        print(f"下载 PSF ID {image_id} 时发生未知错误: {e}")
    finally:
        if response:
            response.close()
    return None
...
if __name__ == "__main__":
    ...
    download_directory = "jplus_psf_data" 
    
    print(f"准备从 {csv_file_path} 读取TILE_ID列表，下载PSF文件至 {download_directory}/")
        ...
        with open(csv_file_path, mode='r', newline='', encoding='utf-8-sig') as csvfile:
            ...
            for row_number, data_row in enumerate(reader, 1):
                ...
                try:
                    ...
                    print(f"处理行 {row_number}: 下载 PSF for TILE_ID={tile_id}")
                                        
                    downloaded_file = download_jplus_psf(
                        session, 
                        tile_id, 
                    ...
                    if downloaded_file:
                        ...
                        print(f"  失败: PSF for TILE_ID={tile_id}")
                        failed_downloads += 1
