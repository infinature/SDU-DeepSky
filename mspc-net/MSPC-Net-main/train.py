...
def main():
    ...
    # 2. load data
    print(f"配置文件中的 data_dir: {cfg['data_dir']}")
    print(f"配置文件中的 class_names: {cfg['class_names']}")
    print(f"配置文件中的 spectrum_length: {cfg['spectrum_length']}")
    ...
    if cfg["test"] != 0:
        ...
        # 新增：打印评估结果（用于被后端 subprocess 捕获）,用于模型测试页面
        metrics = trainer.callback_metrics
        result = {
            "accuracy": round(metrics["test_acc"].item() * 100, 2),
            "f1_score": round(metrics["test_f1"].item() * 100, 2),
            "precision": round(metrics["test_precision"].item() * 100, 2),
            "recall": round(metrics["test_recall"].item() * 100, 2),
        }
        import json
